...
resource "aws_subnet" "recon_subnet" {      # Defines the subnet which the recon machine will reside in.
    ...
    map_public_ip_on_launch = "false"       # Specifies that the instances launched into this subnet should not be assigned a public IP address.
...
resource "aws_internet_gateway" "proxy_internet_gateway" {      # Defines the Internet gateway, which allows internet traffic to and from the VPC. This sits outside the subnets, but can only be directly accessed via the proxy subnet.
    vpc_id = aws_vpc.main_vpc.id

    tags = {
      Name = "internet gateway for proxies"
    }
}
...
resource "aws_route_table" "public_route_table" {     # Defines which external IPs are accessable from the Internet gateway, or in this case, the proxy subnet (public subnet).
  vpc_id = aws_vpc.main_vpc.id
  route = {
...
resource "aws_route_table_association" "public_route_table_association" {     # Creates an association between a route table and a subnet. So in this case, the route table above relates to the proxy subnet, i.e. the proxy subnet can access all external IPs.
